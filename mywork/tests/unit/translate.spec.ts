 ...
 * @LastEditors: hvinci
 * @LastEditTime: 2023-11-19 22:16:15
 * @Description: 
 * 
...
test("init:", () => {
    expect(init(ast, { $account: "", $name: "" })).toEqual({
        nowStepID: "welcome",
        variable: {
            $account: "",
            $name: "",
...
test("test 查询咖啡种类和价格:", () => {
    expect(
        ...
        finish: false,
        message: "想了解我们的咖啡种类和价格吗？我们提供的咖啡有: 马斯卡彭生酪拿铁 18元/杯、生椰拿铁 18元/杯、酱香拿铁 19元/杯、圣诞烤布丁拿铁 21元/杯、太妃真香厚乳拿铁 19元/杯。",
        time: 0,
    });
})
...
test("test 店铺信息:", () => {
    expect(
        ...
        finish: false,
        message: "需要了解附近店铺的位置和营业时间吗？告诉我您所在的城市，我会为您提供相关信息。",
        time: 0,
    });
})
...
test("test 投诉与建议:", () => {
    expect(
        ...
        finish: false,
        message: "如果您有任何投诉或建议，我们非常重视。请告诉我您的问题，我将尽力帮助您或转接您至相关部门。",
        time: 0,
    });
})
...
test("test 常见问题解答:", () => {
    expect(
        ...
        finish: false,
        message: "有关账户、支付、配送等方面的常见问题，您可以直接问我，我会提供尽可能详细的解答。比如,您可以问我:如何创建账户、支付方式有哪些、配送服务说明",
        time: 0,
    });
})
...
test("test silence when afterPlaceOrder:", () => {
    expect(
        translate(
            ast,
            {
...
test("Test interpret: Calculate error", () => {
    expect(() =>
        translate(
            ast,
            {
...
test("status.nowStepID = defaultList.stepID;:", () => {
    expect(
        translate(
            ast,
            {
